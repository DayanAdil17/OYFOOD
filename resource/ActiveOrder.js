...
export default function ActiveOrder(props) {
    ...
    function ListOrder(props){
        ...
        const finish_this_order = (event,id) => {
            event.preventDefault();
            fetch(ip_address+"OYFOOD/back-end/finish_order.php", {
                method:'POST',
                headers:{
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
        ...
        const cancel_this_order = (event,id) => {
            event.preventDefault();
            fetch(ip_address+"OYFOOD/back-end/cancel_order.php", {
                method:'POST',
                headers:{
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
            ...
            <View style = {{justifyContent:'center', alignItems:'center', flexDirection:'column',width:"100%", marginBottom:10}}>
                <Card containerStyle={{width:"90%"}}>
                    ...
                    <View style = {{flex:1,width:"100%",marginTop:20}}>
                        <Button title="FINISH ORDER" onPress={(event) => { finish_this_order(event,data.order_id) }}/>                        
                    </View>
                    <View style = {{flex:1,width:"100%",marginTop:20}}>
                        <Button title="CANCEL ORDER" buttonStyle={{backgroundColor:'#FC431B'}} onPress={(event) => { cancel_this_order(event,data.order_id) }}/>                        
                    </View>
                </Card>
            </View>
        )
